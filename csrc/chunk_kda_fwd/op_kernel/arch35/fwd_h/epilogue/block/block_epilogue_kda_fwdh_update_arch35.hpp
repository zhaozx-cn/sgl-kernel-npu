...
public:
    ...
    BlockEpilogue(Arch::Resource<ArchTag> &resource)
    {
        ...
        finalOutputUbTensor_ping =
            resource.ubBuf.template GetBufferByByte<FinalStateElement>(UPDATE_SCRATCH_BUF_OFFSET);
    ...
    CATLASS_DEVICE
    void ApplyRowScale(AscendC::LocalTensor<float> matrix, AscendC::LocalTensor<float> rowScale, uint32_t rows,
                       uint32_t cols)
    {
        ...
        AscendC::VF_CALL<detail::ApplyRowScaleDualIssue>(matrixAddr, rowScaleAddr, 0, static_cast<uint16_t>(rows),
                                                         static_cast<uint16_t>(cols));
        AscendC::PipeBarrier<PIPE_V>();
    }

    CATLASS_DEVICE
    void PrepareKGate(AscendC::LocalTensor<float> gateOutput, AscendC::LocalTensor<GElementInput> gateInput,
                      uint32_t count)
    {
        __ubuf__ float *gateOutputAddr = reinterpret_cast<__ubuf__ float *>(gateOutput.GetPhyAddr());
        __ubuf__ GElementInput *gateInputAddr = reinterpret_cast<__ubuf__ GElementInput *>(gateInput.GetPhyAddr());
        AscendC::VF_CALL<detail::PrepareKGateRegbase<GElementInput, true>>(gateOutputAddr, gateInputAddr,
                                                                           static_cast<uint16_t>(count));
        AscendC::PipeBarrier<PIPE_V>();
    }

    template <typename StateElement>
    CATLASS_DEVICE void ApplyKGateUpdate(AscendC::LocalTensor<float> update, AscendC::LocalTensor<StateElement> state,
                                         AscendC::LocalTensor<float> rowScale, uint32_t rows, uint32_t cols)
    {
        __ubuf__ float *updateAddr = reinterpret_cast<__ubuf__ float *>(update.GetPhyAddr());
        __ubuf__ StateElement *stateAddr = reinterpret_cast<__ubuf__ StateElement *>(state.GetPhyAddr());
        __ubuf__ float *rowScaleAddr = reinterpret_cast<__ubuf__ float *>(rowScale.GetPhyAddr());
        AscendC::VF_CALL<detail::ApplyKGateUpdateRegbaseDualIssue<StateElement>>(
            updateAddr, stateAddr, rowScaleAddr, static_cast<uint16_t>(rows), static_cast<uint16_t>(cols));
        AscendC::PipeBarrier<PIPE_V>();
    }

    CATLASS_DEVICE
    void operator()(AscendC::GlobalTensor<HElementOutput> hOutput, AscendC::GlobalTensor<FinalStateElement> finalState,
                    AscendC::GlobalTensor<GElementInput> gInput, AscendC::GlobalTensor<HElementInput> hInput,
                    AscendC::GlobalTensor<float> hUpdateInput, AscendC::GlobalTensor<GElementInput> gkInput,
                    AscendC::GlobalTensor<FinalStateElement> initialState, uint32_t chunkSize, uint32_t kHeadDim,
                    uint32_t vBlockDim, uint32_t vHeadDim, Arch::CrossCoreFlag cube2Done, bool isInitialState,
                    bool isFinalState, bool storeFinalState, bool useInitialState, bool isPing, bool cube2AlreadyWaited,
                    bool useDirectFp32Ub, uint64_t directUbFreeFlagBegin, uint64_t directUbReadyFlagBegin,
                    uint64_t directUbFlagStride)
    {
        ...
        if (rowBegin >= mActual) {
            if (useDirectFp32Ub) {
                ...
                uint64_t directUbFlagOffset = directUbSlot + AscendC::GetSubBlockIdx() * directUbFlagStride;
                AscendC::CrossCoreWaitFlag<0x4, PIPE_V>(directUbReadyFlagBegin + directUbFlagOffset);
                AscendC::CrossCoreSetFlag<0x4, PIPE_V>(directUbFreeFlagBegin + directUbFlagOffset);
        ...
        AscendC::LocalTensor<FinalStateElement> finalOutputUbTensor =
            isPing ? finalOutputUbTensor_ping : finalOutputUbTensor_pong;
        ...
        bool useFp32StateUpdate =
            storeFinalState && std::is_same<FinalStateElement, float>::value && (!isInitialState || useInitialState);
        float muls = 1.0f;
        ...
        for (uint32_t rowStart = rowBegin; rowStart < rowEnd; rowStart += ROW_TILE) {
            ...
            AscendC::LocalTensor<float> hUpdateUbTensorThisTile =
                useDirectFp32Ub ? hUpdateUbTensor[(rowStart - rowBegin) * nActual] : hUpdateUbTensor;
            ...
            if constexpr (std::is_same<FinalStateElement, float>::value) {
                if (useFp32StateUpdate) {
                    if (isInitialState) {
                        CopyGmToUb(calcUbTensor, initialState[rowStart * outputStride], rowsThisTile, nActual,
                                   outputStride);
                    } else {
                        CopyGmToUb(calcUbTensor, finalStateThisTile, rowsThisTile, nActual, outputStride);
                    }
                } else {
            ...
            if constexpr (std::is_same<FinalStateElement, float>::value) {
                if (storeFinalState) {
                    if (!isFinalState) {
                        AscendC::Cast(hUbTensor, hUpdateUbTensorThisTile, AscendC::RoundMode::CAST_RINT,
                ...
                if (storeFinalState && isFinalState) {
                    AscendC::Cast(finalOutputUbTensor, hUpdateUbTensorThisTile, AscendC::RoundMode::CAST_RINT,
                                  rowsThisTile * nActual);
                    AscendC::PipeBarrier<PIPE_V>();
                    AscendC::SetFlag<AscendC::HardEvent::V_MTE2>(EVENT_ID0 + pingpongFlag);
                    AscendC::SetFlag<AscendC::HardEvent::V_MTE3>(EVENT_ID2 + pingpongFlag);
        ...
        if (useDirectFp32Ub) {
            uint32_t directUbSlot = isPing ? 0 : 1;
            uint64_t directUbFlagOffset = directUbSlot + AscendC::GetSubBlockIdx() * directUbFlagStride;
            AscendC::CrossCoreSetFlag<0x4, PIPE_MTE3>(directUbFreeFlagBegin + directUbFlagOffset);
        }
