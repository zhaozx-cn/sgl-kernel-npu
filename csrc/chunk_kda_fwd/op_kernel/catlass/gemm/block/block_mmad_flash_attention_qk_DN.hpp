...
template <
    ...
    class TileMmad_>
struct BlockMmadTla<
    MmadFlashAttentionQK<ArchTag_, PAGED_CACHE_FLAG_, NZ_LAYOUT_FLAG_, PA_BNNBSD_FLAG_, true>, L1TileShape_,
...
public:
    ...
    __aicore__ inline void loadQGM(TensorA& gATensor, GemmCoord actualOriShape)
    {
        ...
        auto l1ATensorTlaTile = GetTile(l1ATensorTla, tla::MakeCoord(0, 0), tla::MakeShape(embed, rowNum));
        auto gATensorTlaTile = GetTile(gATensor, tla::MakeCoord(0, 0), tla::MakeShape(embed, rowNum));
        ...
        for (uint32_t nL0Itr = 0; nL0Itr < nL0LoopNum; nL0Itr++) {
            ...
            if constexpr (PAGED_CACHE_FLAG_) {
                if constexpr (!PA_BNNBSD_FLAG_) {
                    ...
                    auto gBTensorTlaTile = GetTile(
                        gBTensor, tla::MakeCoord(blockIdx * blockSize + blockOffset, 0),
                    ...
                    if (isShrink == 1) {
                        ...
                        if (kvSTileIdx == 0) {
                            ...
                            auto blockIdx = gBlockTable.GetValue(blockTableIdx);
                            auto gBTensorTlaTile = GetTile(
                                gBTensor, tla::MakeCoord((blockIdx * blockSize) * kvHeads + blockOffset, 0),
                        ...
                        if (preRemain > 0) {
                            ...
                            auto l1BTensorTlaTile1 = GetTile(
                                l1BTensorTlaTile, tla::MakeCoord(globalRemain, 0), tla::MakeShape(preRemain, embed));
                            if constexpr (NZ_LAYOUT_FLAG_) {
                                CopyGmToL1BwithNZ(
                        ...
                        if (preRemain + globalRemain < 128 && preRemain + globalRemain < l0TileNAct) {
                            ...
                            auto l1BTensorTlaTile2 = GetTile(
                                l1BTensorTlaTile, tla::MakeCoord(preRemain + globalRemain, 0),
                                tla::MakeShape(lastRemain, embed));
                            if constexpr (NZ_LAYOUT_FLAG_) {
                                CopyGmToL1BwithNZ(
            ...
            for (uint32_t mL0Itr = 0; mL0Itr < mL0LoopNum; mL0Itr++) {
                ...
                auto l0CTensorTlaTile = GetTile(
                    l0CTensorTla, tla::MakeCoord(0, mL0Itr * L0_TILE_M), tla::MakeShape(l0TileNAct, l0TileMAct));
                for (uint32_t kL0Itr = 0; kL0Itr < kL0LoopNum; kL0Itr++) {
                    ...
                    auto l1ATensorTlaTile = GetTile(
                        l1ATensorTla, tla::MakeCoord(kL0Itr * L0_TILE_K, mL0Itr * L0_TILE_M),
                    ...
                    auto l1BTensorTlaTile = GetTile(
                        l1BTensorTla, tla::MakeCoord(0, kL0Itr * L0_TILE_K), tla::MakeShape(l0TileNAct, l0TileKAct));
                    ...
                    tileMmad(
                        l0CTensorTlaTile, l0ATensorTla, l0BTensorTla, l0TileNAligned, l0TileMAct, l0TileKAct, initMmad);
            ...
            if constexpr (std::is_same_v<ElementC, half>) {
                ...
                auto ubCTensorTlaTile = GetTile(
                    ubCTensor, tla::MakeCoord(nL0Itr * L0_TILE_N, 0), tla::MakeShape(nFixPAligned16, mPerSubCore));
                ...
                auto ubCTensorTlaTile = GetTile(
                    ubCTensor, tla::MakeCoord(nL0Itr * L0_TILE_N, 0), tla::MakeShape(nFixPAligned8, mFixPAligned32));
