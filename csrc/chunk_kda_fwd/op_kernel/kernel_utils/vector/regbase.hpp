...
constexpr static CastTrait ctFp322HalfZero = {RegLayout::ZERO, SatMode::NO_SAT, MaskMergeMode::MERGING,
                                              AscendC::RoundMode::CAST_ROUND};
constexpr static CastTrait ctFp322HalfOne = {RegLayout::ONE, SatMode::NO_SAT, MaskMergeMode::ZEROING,
                                             AscendC::RoundMode::CAST_ROUND};
...
template <typename TType>
__simd_callee__ inline void HalfOrFloat2Float(RegTensor<float> &dstReg, RegTensor<TType> &srcReg, MaskReg &maskHalf,
                                              MaskReg &maskFloat)
{
    if constexpr (!std::is_same<TType, float>()) {
...
__simd_callee__ inline void CompareTwoReg(MaskReg &maskZeroReg, MaskReg &maskOneReg, RegTensor<TType> &cmp1ZeroReg,
                                          RegTensor<TType> &cmp1OneReg, RegTensor<TType> &cmp2ZeroReg,
                                          RegTensor<TType> &cmp2OneReg, MaskReg &mask)
{
...
__simd_callee__ inline void SelectTwoReg(RegTensor<TType> &dstZeroReg, RegTensor<TType> &dstOneReg,
                                         ...
                                         RegTensor<TType> &src2ZeroReg, RegTensor<TType> &src2OneReg, MaskReg &mask1,
                                         MaskReg &mask2)
{
...
template <typename TType>
__simd_callee__ inline void StoreUnAlignOut(__ubuf__ TType *dstUb, RegTensor<float> &srcReg, MaskReg &mask,
                                            UnalignRegForStore &uStore, uint32_t copyNum)
{
