...
template <class ArchTag_, bool ENABLE_UNIT_FLAG_, bool USE_HF32_MODE_, uint32_t L0C_STAGES_, bool ENABLE_L1_RESIDENT_,
          ...
          class TileMmad_>
struct BlockMmadTla<MmadPingpongTlaMulti<ArchTag_, ENABLE_UNIT_FLAG_, USE_HF32_MODE_, L0C_STAGES_, ENABLE_L1_RESIDENT_,
...
public:
                                   ...
                                   TensorBias const &tensorBias = {}, bool clearL1Padding = false)
    {
        ...
        if (clearL1Padding) {
            AscendC::InitConstValueParams<ElementA> clearParams(1, static_cast<uint16_t>(L1A_TILE_SIZE / 32), 0,
        ...
        if constexpr (ENABLE_L1_RESIDENT) {
            ...
            if (lastAddrA[l1AListId] != tensorTileA.data().GetPhyAddr() ||
                ...
                lastCoordA[l1AListId] = MatrixCoord{static_cast<uint32_t>(tla::get<0>(tensorTileA.coord())),
        ...
        if (clearL1Padding) {
            AscendC::InitConstValueParams<ElementB> clearParams(1, static_cast<uint16_t>(L1B_TILE_SIZE / 32), 0,
        ...
        if constexpr (ENABLE_L1_RESIDENT) {
            if (lastAddrB[l1BListId] != tensorTileB.data().GetPhyAddr() ||
                ...
                lastCoordB[l1BListId] = MatrixCoord{static_cast<uint32_t>(tla::get<0>(tensorTileB.coord())),
                ...
                if constexpr (ENABLE_L1_RESIDENT) {
                    if (lastAddrA[l1AListIdNext] != tensorTileA.data().GetPhyAddr() ||
                        ...
                        lastCoordA[l1AListIdNext] =
                            MatrixCoord{static_cast<uint32_t>(tla::get<0>(tensorTileA.coord())),
                                        static_cast<uint32_t>(tla::get<1>(tensorTileA.coord()))};
                ...
                if constexpr (ENABLE_L1_RESIDENT) {
                    if (lastAddrB[l1BListIdNext] != tensorTileB.data().GetPhyAddr() ||
                        ...
                        lastCoordB[l1BListIdNext] =
                            MatrixCoord{static_cast<uint32_t>(tla::get<0>(tensorTileB.coord())),
                                        static_cast<uint32_t>(tla::get<1>(tensorTileB.coord()))};
