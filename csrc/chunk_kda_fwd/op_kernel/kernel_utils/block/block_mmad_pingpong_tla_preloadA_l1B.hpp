...
template <class ArchTag_, bool ENABLE_UNIT_FLAG_ = false, uint32_t L0C_STAGES_ = 1, uint32_t L1A_STAGES_ = 2,
...
template <class ArchTag_, bool ENABLE_UNIT_FLAG_, uint32_t L0C_STAGES_, uint32_t L1A_STAGES_, uint32_t L1B_STAGES_,
          uint32_t L0A_STAGES_, uint32_t L0B_STAGES_, class L1TileShape_, class L0TileShape_, class ElementA_,
          class ElementB_, class ElementC_, class ElementBias_, class TileCopy_, class TileMmad_>
struct BlockMmadTla<MmadPingpongTlaPreloadAL1B<ArchTag_, ENABLE_UNIT_FLAG_, L0C_STAGES_, L1A_STAGES_, L1B_STAGES_,
                                               L0A_STAGES_, L0B_STAGES_>,
                    L1TileShape_, L0TileShape_, ElementA_, ElementB_, ElementC_, ElementBias_, TileCopy_, TileMmad_> {
public:
    // Type Aliases
    using DispatchPolicy = MmadPingpongTlaPreloadAL1B<ArchTag_, ENABLE_UNIT_FLAG_, L0C_STAGES_, L1A_STAGES_,
            ...
            for (int mL0Idx = 0; mL0Idx < mL0Loop; mL0Idx++) {
                ...
                for (int kL0Idx = 0; kL0Idx < kL0Loop; kL0Idx++) {
                    ...
                    for (int nL0Idx = 0; nL0Idx < nL0Loop; nL0Idx++) {
                        ...
                        auto tensorTileL1B = GetTile(tensorB, tla::MakeCoord(kL0Idx * L0_TILE_K, nL0Idx * L0_TILE_N),
