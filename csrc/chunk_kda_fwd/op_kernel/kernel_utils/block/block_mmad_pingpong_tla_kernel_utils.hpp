...
template <class DispatchPolicy, class L1TileShape, class L0TileShape, class ElementA, class ElementB, class ElementC,
          ...
          class TileCopy = Catlass::Gemm::Tile::PackedTileCopyTla<
              typename DispatchPolicy::ArchTag, ElementA, Catlass::layout::RowMajor, ElementB,
              Catlass::layout::RowMajor, ElementC, Catlass::layout::RowMajor, ElementBias>,
          class TileMmad = Catlass::Gemm::Tile::TileMmadTla<typename DispatchPolicy::ArchTag, ElementA,
...
template <class ArchTag_, bool ENABLE_UNIT_FLAG_, bool USE_HF32_MODE_, uint32_t L0C_STAGES_, bool ENABLE_L1_RESIDENT_,
          uint32_t L1A_STAGES_, uint32_t L1B_STAGES_, uint32_t L0A_STAGES_, uint32_t L0B_STAGES_, uint32_t UB_STAGES_,
...
public:
    ...
    CATLASS_DEVICE void operator()(TensorA &tensorA, TensorB &tensorB, TensorC &tensorC,
                                   Catlass::GemmCoord const &actualShape, TensorBias const &tensorBias = {})
    {
        // Check L1TileShape
        if constexpr (HAS_BIAS) {
    ...
    CATLASS_DEVICE void operator()(TensorA &tensorA, TensorB &tensorB, TensorC tensorCList[MAX_CUBE_VEC_SYNC_NUM],
                                   Catlass::GemmCoord const &actualShape, uint32_t ubRowNum, uint8_t beginSubBlockIdx,
                                   uint64_t l0C2UBAIVAICFlag, uint64_t l0C2UBAICAIVFlag, uint32_t &ubListId,
                                   uint8_t sendVecNum, uint64_t ubNum = 2, TensorBias const &tensorBias = {})
    {
        // Check L1TileShape
        if constexpr (HAS_BIAS) {
            static constexpr uint32_t BIAS_BUF_SIZE = L0_TILE_N * sizeof(ElementAccumulator);
        ...
        if constexpr (!ENABLE_UNIT_FLAG) {
            ...
            for (uint32_t i = 0; i < tileNum; i++) {
                auto tensorTileL0C =
                    GetTile(tensorL0C, tla::MakeCoord(rowIdx, 0), tla::MakeShape(curRowNum, nL1Actual));
                if (sendVecNum == 2) {
